...
void AGolfPlayerController::AddToShotHistory(APaperGolfPawn* PaperGolfPawn)
{
	if (!PaperGolfPawn)
	{
		UE_VLOG_UELOG(this, LogPaperGolfGame, Warning, TEXT("%s: AddToShotHistory - PaperGolfPawn is NULL"), *GetName());
		return;
	}
	...
	{
		UE_VLOG_UELOG(this, LogPaperGolfGame, Log, TEXT("%s: AddToShotHistory - PaperGolfPawn=%s - New shot added to history: %s; Count=%d"),
			*GetName(),
	...
	{
		UE_VLOG_UELOG(this, LogPaperGolfGame, Log, TEXT("%s: AddToShotHistory - PaperGolfPawn=%s - Duplicate shot not added to history: %s; ExistingIndex=%d; Count=%d"),
			*GetName(),
			*PaperGolfPawn->GetName(),
			*ActorLocation.ToCompactString(),
			Index,
...
{
	UE_VLOG_UELOG(this, LogPaperGolfGame, Log, TEXT("%s: SnapToGround"), *GetName());
...
void AGolfPlayerController::SetPaperGolfPawnAimFocus()
{
	...
	{
		UE_VLOG_UELOG(this, LogPaperGolfGame, Log, TEXT("%s-%s: SetPaperGolfPawnAimFocus: LOS to DefaultFocus; Setting to %s"),
	...
	else
	{
		...
		for (auto FocusTarget : FocusableActors)
		{
			...
			{
				UE_VLOG_UELOG(this, LogPaperGolfGame, Verbose, TEXT("%s: SetPaperGolfPawnAimFocus - Skipping target=%s as it is behind"),
					*GetName(), *FocusTarget->GetName());
				UE_VLOG_ARROW(this, LogPaperGolfGame, Verbose, Position, FocusTarget->GetActorLocation(), FColor::Orange, TEXT("Target: %s"), *FocusTarget->GetName());
		...
		UE_VLOG_UELOG(this, LogPaperGolfGame, Log, TEXT("%s-%s: SetPaperGolfPawnAimFocus: BestFocus=%s"),
			*GetName(), *PaperGolfPawn->GetName(), *LoggingUtils::GetName(BestFocus));
		if (BestFocus)
		{
			UE_VLOG_ARROW(this, LogPaperGolfGame, Log, Position, BestFocus->GetActorLocation(), FColor::Blue, TEXT("Target: %s"), *BestFocus->GetName());
		}
	}
...
void AGolfPlayerController::DetermineIfCloseShot()
{
	...
	UE_VLOG_UELOG(this, LogPaperGolfGame, Log, TEXT("%s-%s: DetermineIfCloseShot: %s - Distance=%fm"),
...
void AGolfPlayerController::OnScored()
{
	...
	UE_VLOG_UELOG(this, LogPaperGolfGame, Display, TEXT("%s-%s: OnScored: NumStrokes=%d"),
...
bool AGolfPlayerController::HasLOSToFocus(const FVector& Position, const AActor* FocusActor) const
{
	...
	UE_VLOG_ARROW(this, LogPaperGolfGame, Verbose, Position + 200.f, FocusActor->GetActorLocation() + 200.f,
...
void AGolfPlayerController::CheckAndSetupNextShot()
{
	if (!IsFlickedAtRest())
	{
		UE_VLOG_UELOG(this, LogPaperGolfGame, VeryVerbose,
...
void AGolfPlayerController::ProcessShootInput()
{
	...
	if (!bCanFlick)
	{
		UE_VLOG_UELOG(this, LogPaperGolfGame, Verbose,
...
void AGolfPlayerController::HandleOutOfBounds()
{
	...
	UE_VLOG_UELOG(this, LogPaperGolfGame, Log, TEXT("%s: HandleOutOfBounds"), *GetName());
...
{
	UE_VLOG_UELOG(this, LogPaperGolfGame, Log, TEXT("%s: ResetShotAfterOutOfBounds"), *GetName());
